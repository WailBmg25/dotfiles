...
# Check for AUR updates (timeout guards against AUR RPC / mirror hangs)
aur=$(timeout 15 ${aurhlpr} -Qua 2>/dev/null | wc -l)
ofc=$( (while pgrep -x checkupdates > /dev/null ; do sleep 1; done) ; timeout 15 checkupdates 2>/dev/null | wc -l)
